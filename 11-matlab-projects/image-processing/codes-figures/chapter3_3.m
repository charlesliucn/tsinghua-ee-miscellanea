...
for i=1:len                     %隐藏到图像中，用信息位逐一替换图像各像素点的灰度最低位
    bit=deci2bin(QC(1,i));  	%获取图像像素点的灰度值，并转换为二进制数组
...
for i=1:len                     %len为隐藏信息的字符串长度
    bit=deci2bin(CoMat(1,i));     %对该长度范围内的每一像素点的灰度值进行处理
...
imshow(hallg);              %作出原图
title('原图');              %设定标题
imwrite(hallg,'./chapter3_3/1原图.bmp');  %另存用图片浏览器浏览
subplot(1,2,2);            	%子图2
imshow(hallg_rec);          %作出经过JPEG编码和解码之后复原得到的图
title('JPEG编解码复原图');   %设定标题
imwrite(hallg_rec,'./chapter3_3/2JPEG编解码复原图.bmp');  %另存用图片浏览器浏览
