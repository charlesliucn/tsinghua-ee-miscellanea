...
imwrite(hallp,'./chapter2_3and4_35×35/1原图_35×35.bmp'); 

DCT=dct2(hallp);                    %将hallp进行离散余弦变换
DCT(:,37:40)=0;                     %将DCT系数矩阵右侧4列全部置零
iDCT=idct2(DCT);                    %作离散余弦逆变换
subplot(1,2,2);                     %子图2
imshow(uint8(iDCT));             	%观察DCT系数矩阵改变后处理后的图像
                                    %注意像素值用uint8类型表示，参与浮点运算的像素值要转换为unit8类型
imwrite(uint8(iDCT),'./chapter2_3and4_35×35/2右侧4列置零_35×35.bmp'); 