...
info_bit(find(info_bit==0))=-1; %将二进制数组中的0元素全部更改为-1
for i=1:len                     %隐藏到图像中，用信息位逐一替换图像各像素点的灰度最低位
    block=QC(:,i);              %QC的第i列表示第i块
    last_nonz=find(block~=0,1,'last'); %先找到最后一个非零系数，之后元素均为0元素
    if(last_nonz==64)           %当最后一个(第64个系数)非零时，用信息位替换
        QC(64,i)=info_bit(i);
    else                        %其他情况，将信息位追加在最后一个非零DCT系数之后
...
for i=1:len                             %len为隐藏信息的字符串长度
    block=CoMat(:,i);                   %CoMat第i列表示第i块
    last_nonz=find(block~=0,1,'last');  %先找到最后一个非零系数，之后元素均为0元素
    if(last_nonz==64)                   %CoMat某一列最后一个DCT系数非零时，该系数是信息位
        exc(i)=CoMat(64,i);             
    else
        exc(i)=CoMat(last_nonz,i);      %其他情况，CoMat该列的最后一个非零系数是信息位
    end
end
exc(find(exc==-1))=0;                   %将1，-1序列还原为二进制数组形式
...
imshow(hallg);              %作出原图
title('原图');              %设定标题
imwrite(hallg,'./chapter3_2c/1原图.bmp');  %另存用图片浏览器浏览
subplot(1,2,2);            	%子图2
imshow(hallg_rec);          %作出经过JPEG编码和解码之后复原得到的图
title('JPEG编解码复原图');   %设定标题
imwrite(hallg_rec,'./chapter3_2c/2JPEG编解码复原图.bmp');  %另存用图片浏览器浏览
