function AIS_LogConst = AIS2(W,a,b,M_run,beta,batchdata)
% AIS(Annealed Importance Sampling)算法估计RBM归一化常数(使用batchdata)
% 输入：
...
%   batchdata：数据分成batch处理.
% 输出：
%   AIS_LogConst：AIS算法得到的归一化常数估计值.

%% 根据测试数据对base-rate model的可见层bias进行初始化
[num_batch1,numofvis,num_batch2] = size(batchdata);	%获取batchdata的基本信息
count = zeros(numofvis,1);                       	%初始化隐藏层unit
for i=1:num_batch2
    temp = sum(batchdata(:,:,i));                   
    count = count + temp';
end
interval = 5;
prob = (count + interval*num_batch2)/(num_batch1*num_batch2 + interval*num_batch2);
b_base = (log(prob) - log(1-prob))';          %base-rate model RBM的可见层bias全部取由batchdata得到的结果
